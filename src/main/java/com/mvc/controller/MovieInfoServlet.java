...
public class MovieInfoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if ("list".equals(uri)) {
			request.setAttribute("movieList", miRepo.selectMovieList());
			path += "movie-info/list.jsp";
		} else if ("view".equals(uri)) {
			String miNum = request.getParameter("miNum");
			request.setAttribute("movie", miRepo.selectMovieListOne(miNum));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if("insert".equals(uri)) {
			...
			int result = miRepo.insertMovieInfo(movieInfoMap);
